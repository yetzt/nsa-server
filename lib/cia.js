...
cia.prototype.parse = function(message, remote, callback) {
	...
	var validate = function(message){
		...
		if (message.hasOwnProperty("defcon")) {
			message.defcon = parseInt(message.defcon,10);
			if (isNaN(message.defcon)) message.defcon = 0;
			if (message.defcon < 0) message.defcon = 0;
			if (message.defcon > 5) message.defcon = 5;
		}
...
cia.prototype.handle = function(message) {
	...
	if (!this.nodes.hasOwnProperty(message.id)){
		...
		this.timers[message.id] = setInterval(function(){
			...
			if (self.nodes[message.id].active && self.nodes[message.id].lastbeat > (self.nodes[message.id].interval * 3)) { // FIXME: make this adjustable?
				...
				self.emit("node+inactive", message.id, message.service, message.node);
			} else {
		...
		this.emit("node+register", message.id, message.service, message.node);
	...
	if (this.nodes[message.id].active === false) {
		...
		self.emit("node+active", message.id, message.service, message.node);
	}
	...
	if (message.sequence < this.nodes[message.id].sequence) {
		...
		self.emit("node+reset", message.id, message.service, message.node);
	};
	
	this.nodes[message.id].sequence = message.sequence;

	// check for data message
	if (message.type === "data" && message.hasOwnProperty("data")) {
		for (key in message.data) if (message.data.hasOwnProperty(key)) {
			this.nodes[message.id].data[key] = message.data[key];
			this.nodes[message.id].lastdata = (new Date()).getTime();
		}
		self.emit("node+data", message.id, message.service, message.node);
	};
	
	// check for defcon message
	if (message.type === "defcon" && message.hasOwnProperty("defcon")) {
		this.nodes[message.id].defcon = message.defcon;
		this.nodes[message.id].lastdata = (new Date()).getTime();
		self.emit("node+defcon", message.id, message.service, message.node);
	};
	...
	if (message.type === "retire") {
		...
		self.emit("node+retire", message.id, message.service, message.node);
	};
